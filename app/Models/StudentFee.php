...
class StudentFee extends Model
{
    ...
    /**
     * Calculate the tuition fee amount from fee structure
     */
    public function getTotalFeeAttribute()
    {
        return $this->feeStructure?->basic_fee ?? 0;
    }
...
public function processPayment(float $amount, array $paymentData = []): array
{
    ...
    // Update payment amounts (based on tuition/basic fee only)
    $this->amount_paid += $amount;
    $tuitionFee = $this->feeStructure->basic_fee;
    $this->balance = max(0, $tuitionFee - $this->amount_paid);

    // Update payment status
    if ($this->amount_paid <= 0) {
        $this->payment_status = 'unpaid';
    } elseif ($this->amount_paid > $tuitionFee) {
    ...
    if ($this->payment_status === 'overpaid') {
        $overpayment = $this->amount_paid - $tuitionFee;
...
public function getPaymentTimeline(): array
{
    ...
    $timeline[] = [
        ...
        'amount' => $this->feeStructure->basic_fee,
        'running_balance' => $this->feeStructure->basic_fee,
    ];

    // Add all payment transactions
    $runningBalance = $this->feeStructure->basic_fee;
