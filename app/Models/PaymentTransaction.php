...
class PaymentTransaction extends Model
{
    ...
    protected static function boot()
    {
        ...
        // Generate reference number if not provided and auto-populate academic_year_id
        static::creating(function ($transaction) {
            if (!$transaction->reference_number) {
                $transaction->reference_number = $transaction->generateReferenceNumber();
            }

            // Auto-populate academic_year_id from student fee if not set
            if (!$transaction->academic_year_id && $transaction->student_fee_id) {
                $studentFee = StudentFee::find($transaction->student_fee_id);
                if ($studentFee) {
                    $transaction->academic_year_id = $studentFee->academic_year_id;
                }

                // Fallback to active academic year
                if (!$transaction->academic_year_id) {
                    $transaction->academic_year_id = AcademicYear::where('is_active', true)->first()?->id;
                }
            }
        });
    }
