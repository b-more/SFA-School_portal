                    ...
                    Forms\Components\Wizard\Step::make('Select Recipients')
                        ->schema([
                            Forms\Components\Select::make('recipient_scope')
                                ->label('Audience')
                                ->options([
                                    'parents' => 'Parents/Guardians (SMS)',
                                    'all_staff' => 'All Staff (Portal Notice)',
                                    'teachers' => 'Teachers Only (Portal Notice)',
                                ])
                                ->default('parents')
                                ->required()
                                ->live()
                                ->helperText(function (Forms\Get $get) {
                                    $scope = $get('recipient_scope');
                                    if ($scope === 'parents') {
                                        return 'SMS will be sent to parents\' phone numbers';
                                    }
                                    return 'Message will appear in staff Communication Center with a notification badge';
                                }),
                            ...
                            Forms\Components\Select::make('recipient_type')
                                ...
                                ->default('parents')
                                ->required()
                                ->visible(fn (Forms\Get $get) => $get('recipient_scope') === 'parents')
                                ->live(),
                            ...
                            Forms\Components\Section::make('Filter Options')
                                ...
                                ->visible(fn (Get $get): bool => $get('recipient_scope') === 'parents'),
                            ...
                            Forms\Components\Actions::make([
                                Forms\Components\Actions\Action::make('preview_recipients')
                                    ...
                                    ->visible(fn (Get $get) => $get('recipient_scope') === 'parents'),
                            ]),
    ...
    public function create_broadcast()
    {
        ...
        try {
            ...
            $broadcast = MessageBroadcast::create([
                ...
                'filters' => [
                    'recipient_type' => $data['recipient_type'] ?? 'parents',
                    'grade_id' => $data['grade_id'] ?? null,
                    'fee_status' => $data['fee_status'] ?? null,
                ],
                'total_recipients' => $isStaffNotice ? 0 : $this->recipientCount,
                'status' => $isStaffNotice ? 'completed' : 'draft',
                'completed_at' => $isStaffNotice ? now() : null,
                'created_by' => Auth::id(),
            ]);
            ...
            if ($isStaffNotice) {
                Notification::make()
                    ...
                    ->body('Your notice has been published. Staff will see it in their Communication Center with a notification badge.')
                    ->success()
                    ->send();

                $this->redirect(CommunicationCenterResource::getUrl('index'));
            } else {
                Notification::make()
