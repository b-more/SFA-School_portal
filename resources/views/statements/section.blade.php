...
<body>
    <div class="container">
        ...
        <div class="info-section">
            ...
            @endif
            <p><strong>Section:</strong> {{ $reportData['section']->name }}</p>
        </div>

        <div class="summary-box">
            <div class="summary-title">Section Payment Summary</div>
        ...
        <div class="fee-details">
            <h3>Fee Payment Details by Grade</h3>
            @if($studentFees->isEmpty())
                <p>No fee records found for this section.</p>
            @else
                @php
                ...
                @foreach($feesByGrade as $gradeName => $gradeFees)
                    <div class="grade-heading">{{ $gradeName }}</div>
                    <table class="fee-table">
                        ...
                        <tbody>
                            ...
                            @endforeach
                            <tr class="total-row">
                                <td colspan="4"><strong>Total for {{ $gradeName }}</strong></td>
                                <td>{{ number_format($gradeFees->sum(function($fee) { return $fee->feeStructure->total_fee; }), 2) }}</td>
                                <td>{{ number_format($gradeFees->sum('amount_paid'), 2) }}</td>
                                <td>{{ number_format($gradeFees->sum('balance'), 2) }}</td>
                                <td colspan="2"></td>
                            </tr>
                        </tbody>
                    </table>
                ...
                <table class="fee-table">
                    <tr class="total-row">
                        <td colspan="4"><strong>Section Total</strong></td>
            ...
            <div class="summary-grid">
                <div class="summary-item">
                    ...
                    <p>
                        ...
                        {{ $studentFees->where('payment_status', 'paid')->count() }} students
                        ({{ number_format(($studentFees->where('payment_status', 'paid')->count() / $reportData['count']) * 100, 1) }}%)
                    </p>
                    <p>
                        <strong>Partially Paid:</strong>
                        {{ $studentFees->where('payment_status', 'partial')->count() }} students
                        ({{ number_format(($studentFees->where('payment_status', 'partial')->count() / $reportData['count']) * 100, 1) }}%)
                    </p>
                    <p>
                        <strong>Unpaid:</strong>
                        {{ $studentFees->where('payment_status', 'unpaid')->count() }} students
                        ({{ number_format(($studentFees->where('payment_status', 'unpaid')->count() / $reportData['count']) * 100, 1) }}%)
                    </p>
                </div>

                <div class="summary-item">
                    <h4>Collection Rate</h4>
                    <p><strong>Total Expected:</strong> ZMW {{ number_format($reportData['totalFees'], 2) }}</p>
                    <p><strong>Total Collected:</strong> ZMW {{ number_format($reportData['totalPaid'], 2) }}</p>
                    <p>
